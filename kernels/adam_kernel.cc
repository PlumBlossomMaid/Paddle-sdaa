...
const float* ScalarDataAsFloat(const phi::DenseTensor& tensor, float* storage) {
  ...
  if (tensor.dtype() == phi::DataType::FLOAT64) {
    ...
    if (tensor.place().GetType() == phi::AllocationType::CPU) {
      value = *ptr;
    } else if (sdaaMemcpy(&value, ptr, sizeof(double), sdaaMemcpyDeviceToHost) !=
               sdaaSuccess) {
      value = 0.0;
    }
    *storage = static_cast<float>(value);
    return storage;
  }
  return tensor.data<float>();
}
...
void AdamKernel(
  ...
  TCUS_CHECK(sdcops::pd_adam_out(n_total,
                                 ...
                                 beta1,
                                 beta2,
                                 const_cast<float*>(ScalarDataAsFloat(*beta1_pow, &beta1_pow_host)),
                                 const_cast<float*>(ScalarDataAsFloat(*beta2_pow, &beta2_pow_host)),
                                 epsilon,
                                 const_cast<float*>(ScalarDataAsFloat(*lr, &lr_host)),
                                 device_scale,
                                 b1_out,
                                 b2_out,
                                 dtypes,
...
void AdamwKernel(
  ...
  TCUS_CHECK(sdcops::pd_adamw_out(n_total,
                                  ...
                                  beta1_,
                                  beta2_,
                                  const_cast<float*>(ScalarDataAsFloat(*beta1_pow_, &beta1_pow_host)),
                                  const_cast<float*>(ScalarDataAsFloat(*beta2_pow_, &beta2_pow_host)),
                                  epsilon_,
                                  const_cast<float*>(ScalarDataAsFloat(*lr, &lr_host)),
                                  device_scale,
