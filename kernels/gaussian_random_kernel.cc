...
void GaussianRandomAlign(const Context& dev_ctx,
  ...
  PADDLE_ENFORCE_EQ(status,
                    SDC_SUCCESS,
                    phi::errors::External("SDAA Gaussian random kernel failed."));
  if (needs_cast) {
...
void GaussianRandomKernel(const Context& dev_ctx,
  ...
  if (value && seed == 0) {
    ...
    int ignored_sm_count = 0;
    custom_kernel::GetGPUConfig(value, &ignored_max_threads, &ignored_sm_count);
    VLOG(4) << "CUDA random alignment is not used on SDAA: " << value;
  }
