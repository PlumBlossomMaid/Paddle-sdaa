...
EPOCH_NUM = int(os.getenv("PADDLE_SDAA_MNIST_EPOCHS", "1"))
BATCH_SIZE = int(os.getenv("PADDLE_SDAA_MNIST_BATCH_SIZE", "64"))
MAX_TRAIN_STEPS = int(os.getenv("PADDLE_SDAA_MNIST_MAX_TRAIN_STEPS", "2"))
MAX_TEST_STEPS = int(os.getenv("PADDLE_SDAA_MNIST_MAX_TEST_STEPS", "1"))
NUM_WORKERS = int(os.getenv("PADDLE_SDAA_MNIST_NUM_WORKERS", "0"))
...
def parse_args():
    ...
    parser.add_argument(
        "--device",
        type=str,
        choices=["cpu", "gpu", "sdaa"],
        default="sdaa",
        help="Choose the device to run, it can be: cpu/gpu/sdaa, default is sdaa.",
    )
    parser.add_argument(
...
def test(epoch_id, test_loader, model, cost):
    model.eval()
    avg_acc = [[], []]
    for step_id, (images, labels) in enumerate(test_loader()):
        if step_id >= MAX_TEST_STEPS:
            break
...
def main(args):
    ...
    if MAX_TRAIN_STEPS > 0:
        train_dataset = paddle.io.Subset(
            train_dataset, list(range(min(len(train_dataset), MAX_TRAIN_STEPS * BATCH_SIZE)))
        )
    if MAX_TEST_STEPS > 0:
        test_dataset = paddle.io.Subset(
            test_dataset, list(range(min(len(test_dataset), MAX_TEST_STEPS * BATCH_SIZE)))
        )
    train_loader = paddle.io.DataLoader(
        ...
        drop_last=True,
        use_shared_memory=False,  # test device may not have shared memory
    )
